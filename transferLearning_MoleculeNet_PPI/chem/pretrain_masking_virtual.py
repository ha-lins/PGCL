...
def train(args, model_list, loader, optimizer_list, device):
    model = model_list[0]
    optimizer_model = optimizer_list[0]
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        ## loss for nodes
        pred_node = model.linear_pred_atoms(node_rep[batch.masked_atom_indices])
        ...
        if args.mask_edge:
            ...
            pred_edge = model.linear_pred_bonds(edge_rep)
...
def main():
    ...
    model = GNN_Virtualnode(args.num_layer, args.emb_dim, JK = args.JK, drop_ratio = args.dropout_ratio, gnn_type = args.gnn_type).to(device)
    model_list = [model]
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if epoch % 20 == 0:
            torch.save(model.state_dict(), "./models_graphcl_virtual/masking_virtual" + str(epoch) + ".pth")
